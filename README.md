...
**这只解决了一半**：hosts 文件只处理 DNS 解析，让 Windows 拿到正确的真实 IP，但拿到 IP 之后，"默认全走隧道"这条规则依然会把发往这个 IP 的流量吸进隧道绕去家里——公司服务器一看这个连接不是从公司内网发起的，一样会拒绝（现象跟假 IP 那次很像，但根子已经不是 DNS 了）。完整修复还要**再给这个真实 IP 单独加一条路由，直连本地网关、不走隧道**（跟前面 Windows 分流脚本的默认路由是同一批命令，一起维护）：

```cmd
route add <真实IP> mask 255.255.255.255 <公司本地网关> metric 1 IF !PHYS_IF! -p
```

**踩过的坑**：公司本地网络的网关地址后续变了（换工位、公司网络重新装修），这条路由的下一跳网关没跟着改，导致路由指向一个不存在的下一跳，形同虚设，症状会跟当初没加这条路由时一模一样——排查时容易先怀疑 hosts 文件是不是又被清空了，其实 hosts 没问题，是这条路由的网关过期了。**目标 IP 不用动，只改网关这一个字段。**

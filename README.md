...
### 一个反复出现的根子问题：网关模式转发流量，跟本机流量不是同一套待遇

下面两条踩坑记录（DNS 假 IP 保护、QUIC 卡顿）表面上是两个不相干的症状，**根子其实是同一个**：Surge 对"本机自己发起的流量"（Enhanced Mode）有一整套优化和保护机制（假 IP、UDP 快速拒绝等），但对"网关模式转发进来的流量"（其他设备经路由转发过来的），这些机制大多不生效——网关模式在 Surge 里更像是"透明转发"，享受不到本机流量的那些特殊待遇。**任何在本机验证有效的 Surge 配置，都不能假设对转发流量也同样有效，必须单独用转发设备实测验证。**下面是这个根子问题目前发现的两次具体表现：
...
跟上面 DNS 假 IP 保护那条是**同一类根子问题**的另一次体现——只是这次不是 DNS 解析层面，而是 UDP 协议处理层面对本机流量和转发流量待遇不一致。
...
**先尝试过、对网关转发流量无效的修复**：在 `[Rule]` 最顶部加一条全局规则直接拦截 QUIC：

```
AND,((PROTOCOL,UDP),(DEST-PORT,443)),REJECT
```

这条规则对 **Surge 本机自己发起的流量有效**（本来就有 `udp-policy-not-supported-behaviour = REJECT` 兜底，这条算双保险，可以留着），但对**网关模式转发流量没有效果**——实测抓包确认，加了这条规则之后，转发设备依然在正常发送 UDP 443 流量，问题原样存在。原因跟前面"假 IP 保护"那条一样：Surge 规则这一层，对网关模式转发流量的处理跟本机流量本来就不是同一套逻辑，不能假设在本机验证有效的规则对转发流量也生效。

**真正有效的修复**：换到网络层（NAS 的 `iptables`）去拦截，不依赖 Surge 网关模式怎么处理。因为这台 NAS 的内核没有 `REJECT` target 模块（`Couldn't load target 'REJECT'`），用 DNAT 把流量导向一个没有服务监听的端口，让内核自动回一个 "ICMP port unreachable"，效果等同于主动拒绝（区别于静默丢弃 DROP，能让客户端立刻知道此路不通、马上降级到 TCP，而不是傻等超时）：

```bash
# 加在 wg0.conf 的 PostUp（对应 PostDown 里加一条 -D 的镜像命令用于清理）
iptables -t nat -A PREROUTING -s 10.13.13.0/24 -p udp --dport 443 -j DNAT --to-destination <NAS内网IP>:1
```

**踩过的坑**：这条规则如果直接在宿主机上用 `iptables -A PREROUTING ...` 添加，会被静默错误地插入到 Docker 管理的 `DEFAULT_PREROUTING` 子链里，顶层链完全没变化，规则形同虚设（跟前面 POSTROUTING 那个坑是同一个原因）。必须在**创建 `wg0.conf` 里其他 NAT 规则的同一个容器内部**执行（`docker exec <容器名> iptables -t nat -A PREROUTING ...`），才能正确落到顶层链。

**副作用**：这条规则按源网段（`10.13.13.0/24`，即 WireGuard 客户端子网）全局拦截 UDP 443，不区分域名、不区分国内外。对普通网页浏览没有影响（自动降级 TCP，无感知）；但会连带拦掉依赖 UDP 传输媒体流、且恰好也用了 443 端口的实时通信应用（视频通话、部分游戏），可能导致通话质量下降甚至连不上。加规则后建议实测一下常用的视频通话软件。

### MITM 类去广告模块可能私藏了域名级的 QUIC 拦截规则

跟上面网关模式那条 QUIC 拦截是相关但不同的机制，排查小红书视频加载卡顿时顺带弄清楚的一个通用知识点。

**现象**：某个 App（比如小红书）图片/视频加载卡顿，怀疑是装的去广告模块干的，关掉模块看解密开销问题——但即使确认模块的 MITM 解密开销确实存在，"关掉模块能不能立刻恢复正常"这件事不能想当然，还有一层容易被忽略的机制。

**原理**：MITM 只能对 TCP 上的标准 TLS 握手动手脚（拦截、换证书、解密、改内容、重新加密），没法解密跑在 UDP 上的 QUIC。如果放着 QUIC 不管，App 会直接绕开 TCP/TLS 走 QUIC，MITM 拦截和广告过滤直接失效。所以很多做 MITM 的去广告模块，会顺手给自己要处理的目标域名加一条**只针对这些域名的 QUIC 拦截规则**（类似 `DOMAIN-SUFFIX,目标域名,REJECT` 但专门筛 UDP:443），逼着 App 老实走 TCP，这样 MITM 才能生效。

**这跟前面"网关模式转发流量卡在 QUIC"那条踩坑的区别**：前面那条是给**网关模式转发流量**做的全局兜底规则（`AND,((PROTOCOL,UDP),(DEST-PORT,443)),REJECT`，不区分域名，覆盖所有转发设备）；这里说的是 **MITM 模块自己私藏的、只针对特定域名**的 QUIC 拦截规则，两者可能同时出现在同一份配置里，来源不同、生效范围也不同，容易被当成同一回事排查。

**排查建议**：如果某个 App 卡顿、怀疑是去广告模块导致，除了怀疑模块的解密处理开销本身，还要留意这个模块有没有偷偷带了针对该 App 域名的 QUIC 拦截规则——关闭/卸载模块前后，可以在 Surge 的连接日志里对比同一个域名的连接过程有没有变化（走没走 QUIC、有没有先尝试 UDP 又超时降级到 TCP），比单纯凭感觉判断"卡不卡"更准确。

### Windows 客户端分流脚本里写死的 WireGuard 服务端 IP 会过期

**背景**：Windows 客户端分流脚本（见前面"Windows 客户端分流方案"一节）除了默认路由那条，还有一条容易被忽略的路由——把 WireGuard 服务端自己的公网 IP（`home.ricklxf.top` 解析出来的地址）单独走本地物理网卡直连，不吸进隧道。这条路由是必须的：客户端要连回家握手，这个握手包本身得先离开本机才能到公网，如果被"默认全走隧道"这条规则也吸进去，就会变成"连隧道需要先连上隧道"的死循环。

**踩的坑**：这条路由早期是手动把解析出来的 IP **写死**在脚本里的。家里公网 IP 是动态的，DDNS-GO 会自动帮你把 `home.ricklxf.top` 这个域名更新指向新 IP，但脚本里写死的旧 IP 不会跟着变——域名早就指向新地址了，脚本还在给旧地址开绿灯，新地址反而被"默认全走隧道"吸走，握手包出不去，隧道直接连不上。这个坑不会立刻暴露，家里公网 IP 只要没换过，能正常用很久，一旦运营商换了一次公网 IP，才会突然发现连不上。

**顺带的坑**：公司本地网络的网关地址也可能变（换工位、公司网络重新装修），这条路由的下一跳网关（`IF !PHYS_IF!` 后面那个网关 IP）也得跟着改，不然这条路由本身就是指向一个不存在的下一跳，形同虚设。

**修复**：脚本里不再写死 IP，改成每次运行时用 `nslookup -type=A home.ricklxf.top` 动态解析当前地址，解析结果跟上次不一样就自动把旧路由删掉、加新路由，用同目录下一个小文本文件记录"上次解析到的 IP"用于比对。**必须在修改默认路由之前完成解析**——如果解析放在默认路由改完之后，DNS 查询本身也可能被吸进隧道，造成"解析不出来 → 排除不了 → 隧道更连不上"的死循环，顺序反了等于没修。
...
### 特定设备查询 NAS 上自建服务超时，服务端自己的日志却显示"已处理"：策略路由误伤了 NAS 自己回复同网段设备的流量

**现象**：客户端设备（比如公司电脑）查询部署在 NAS 上的本地服务（如 AdGuard Home DNS）时超时，耗时稳定在十几秒；换查询的域名、换客户端设备，表现不一样（有的设备正常，有的设备必现超时）；服务端自己的日志显示这次查询"已处理"，耗时不到 1 毫秒，看起来完全正常；在 NAS 上抓包能看到客户端的查询包确实到达了，但抓不到任何应答包发出去。

**原因**：如果 NAS 上为了"把 WireGuard 客户端网段（比如 `10.13.13.0/24`）的转发流量分流给另一台设备处理"设置了类似 `ip rule add from 10.13.13.0/24 lookup 100` 这样的策略路由规则——这条规则匹配的是"源地址落在这个网段"，而 NAS 自己的 WireGuard 接口地址（比如 `10.13.13.1`）本身数字上也落在这个网段范围内。当 NAS 自己（不是转发别人的流量，而是自己主动发起，比如本地服务回复客户端的查询）要给同网段的另一台设备回包时，这个包的源地址同样会命中这条规则，被当成"待转发的客户端流量"错误处理、送去了完全不相关的地方，而不是直接从 WireGuard 接口原路送回。这个 bug 只影响"NAS 自己发起、目标是同网段其他设备"这一种流量模式，不影响"客户端访问外网"这种最常见的场景，所以配置好之后长期使用都很难察觉，只有专门测试这条路径时才会暴露。

**验证**：
```bash
# 模拟 NAS 自己（本地服务地址）要回复给某个客户端设备时，内核实际会选哪条路由
ip route get <客户端地址> from <NAS自己的WireGuard接口地址>
```
如果输出显示走了策略路由表（比如 `table 100`）、网关指向了别的设备，而不是直接 `dev wg0`，就是撞上了这个问题。

**修复**：加一条优先级更高（数字更小，ip rule 数字越小优先级越高）的规则，把"NAS 自己发起的流量"从这条策略路由里摘出来，让它老老实实走主路由表：
```bash
ip rule add from <NAS自己的WireGuard接口地址> lookup main priority 100
```

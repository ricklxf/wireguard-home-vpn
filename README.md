...
### Windows 分流脚本里排除特定 IP 的那条路由，网关跟着公司网络变化会过期

**背景**：Windows 客户端分流脚本（见前面"Windows 客户端分流方案"一节）里，默认路由之外通常还会有额外的单个 IP 排除规则——比如公司打印机依赖的云打印服务商地址，需要直连本地网关访问，不走隧道（查过归属地，是托管在阿里云上的服务器，不是公司办公网络自己的出口 IP，只是这个地址是打印功能必需的）。

**踩的坑**：一开始看到这类"单个公网 IP 走本地网关"的路由，很容易先入为主地以为是"给 WireGuard 服务端自己的公网 IP（`home.ricklxf.top`）留的排除路由，用来避免握手包被吸进隧道形成死循环"——这个假设看着合理，但不能不经确认就下结论：这次实际排查后发现，这条路由排除的其实是打印功能依赖的云服务地址，跟 WireGuard 服务端毫无关系。往错误方向定位问题，差点把一条正常工作的路由改错。

**真正的坑只有一个**：公司本地网络的网关地址变了（换工位、公司网络重新装修），这条路由的下一跳网关（`IF !PHYS_IF!` 后面那个网关 IP）没跟着改，导致这条路由指向一个不存在的下一跳，形同虚设。**目标 IP 本身不用动**，只改网关这一个字段就够了。

**教训**：脚本里看到一条排除特定 IP 的路由，先确认这个 IP 实际是什么服务，不要凭"看起来像什么"就假设它的用途——同样是"单个公网 IP 走本地直连"这种写法，可能是给完全不同的服务用的，排查方向想当然地往复杂里猜，反而容易南辕北辙。
